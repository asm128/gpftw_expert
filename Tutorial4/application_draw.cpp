...
#include "llc_bitmap_target.h"
	...
	if( 0 == textureMetrics.x
	 ||	0 == textureMetrics.y
		...
		//}
		interpolatedBGRA														= finalColor + ambientColor;
		srcTexel;//interpolatedBGRA														= ::llc::SColorFloat(srcTexel) * finalColor + ambientColor;
	}
...
//
	...
	for(uint32_t iGNDNode = 0; iGNDNode < 1; ++iGNDNode) {
		...
		for(uint32_t iTriangle = 0, triCount = applicationInstance.GNDModel.Nodes[0].VertexIndices.size(); iTriangle < triCount; ++iTriangle) {
		...
		for(uint32_t iTriangle = 0, triCount = renderCache.Triangle3dIndices.size(); iTriangle < triCount; ++iTriangle) { // 
			...
			for(uint32_t iPixel = 0, pixCount = renderCache.TrianglePixelCoords.size(); iPixel < pixCount; ++iPixel) {
				...
				if(0 == ::drawPixelGND(renderCache, offscreen.View[pixelCoord.y][pixelCoord.x], pixelWeights, triangle3DUVs, applicationInstance.TextureGrid.View, iTriangle, lightDir.Cast<double>()))
					++pixelsDrawn;
				else
					++pixelsSkipped;
			}
