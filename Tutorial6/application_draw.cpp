...
#include "llc_bitmap_target.h"
	...
	for(uint32_t iGNDNode = 0; iGNDNode < applicationInstance.GNDData.TextureNames.size(); ++iGNDNode) {
		const ::llc::SModelNodeGND													& gndNode									= applicationInstance.GNDModel.Nodes[iGNDNode];
		...
		for(uint32_t iTriangle = 0, triCount = renderCache.Triangle3dIndices.size(); iTriangle < triCount; ++iTriangle) { // transform normals
			...
			vertNormalsTri.A														= gndNode.Normals[vertexIndices.A]; //xWorld.TransformDirection(gndNode.Normals[vertexIndices.A]).Normalize();
			vertNormalsTri.B														= gndNode.Normals[vertexIndices.B]; //xWorld.TransformDirection(gndNode.Normals[vertexIndices.B]).Normalize();
			vertNormalsTri.C														= gndNode.Normals[vertexIndices.C]; //xWorld.TransformDirection(gndNode.Normals[vertexIndices.C]).Normalize();
