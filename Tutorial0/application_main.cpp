...
static				::llc::error_t										textCalcSizeLine							(const ::llc::SCoord2<int32_t>& sizeCharCell, const char (&text0)[_sizeString] )	{	// --- This function will draw some coloured symbols in each cell of the ASCII screen.
	return (::llc::error_t)(sizeCharCell.x * ::llc::size(text0) - 1);
}
...
static				::llc::error_t										textDrawFixedSize							(::llc::grid_view<::llc::SColorBGRA>& bmpTarget, const ::llc::bit_array_view<uint32_t>& viewTextureFont, const ::llc::SCoord2<uint32_t> & viewMetrics, uint32_t characterCellsX, int32_t dstOffsetY, const ::llc::SCoord2<int32_t>& sizeCharCell, const ::llc::view_const_string& text0, const ::llc::SCoord2<int32_t> dstTextOffset, const ::llc::SColorBGRA& color)	{	// --- This function will draw some coloured symbols in each cell of the ASCII screen.
	...
	for(int32_t iChar = 0, charCount = (int32_t)text0.size(); iChar < charCount; ++iChar) {
		...
		error_if(errored(::llc::grid_copy_alpha_bit(bmpTarget, viewTextureFont, dstTextOffset + dstOffset1, viewMetrics, ::llc::SColorBGRA{0, 0xFF, 0xFF, 0xFF}, srcRect0)), "I believe this never fails.");
		color;
	}
	return 0;
}
...
template<size_t _sizeString>
static				::llc::error_t										textDrawAlignedFixedSize					(::llc::grid_view<::llc::SColorBGRA>& targetView, const ::llc::bit_array_view<uint32_t>& fontAtlas, const ::llc::SCoord2<uint32_t> & viewMetrics, uint32_t lineOffset, const ::llc::SCoord2<uint32_t>& targetSize, const ::llc::SCoord2<int32_t>& sizeCharCell, const char (&text0)[_sizeString], const ::llc::SColorBGRA& color)	{	// --- This function will draw some coloured symbols in each cell of the ASCII screen.
	...
	return ::textDrawFixedSize(targetView, fontAtlas, viewMetrics, 32, dstOffsetY, sizeCharCell, {text0, ::llc::size(text0) -1}, dstTextOffset, color);
}
...
struct SCamera {
	...
	::llc::SCoord3<float>														cameraFront									= (camera.Target - camera.Position).Normalize();
	::llc::SCoord3<float>														cameraRight									= cameraUp.Cross(cameraFront).Normalize();
	...
	projection.FieldOfView(.25 * ::llc::math_pi, offscreenMetrics.x / (double)offscreenMetrics.y, fNear, fFar );
	...
	for(uint32_t iTriangle = 0; iTriangle < 12; ++iTriangle) {
		...
		::llc::scale(transformedTriangle, {1, 2, 5});
	...
	for(uint32_t iTriangle = 0; iTriangle < 12; ++iTriangle) { // Maybe the scale
		::llc::STriangle3D<float>													& transformedTriangle3D						= triangle3dList[iTriangle];
	...
	for(uint32_t iTriangle = 0; iTriangle < 12; ++iTriangle) {
		...
		triangle3dColorList[iTriangle]											= ::llc::BLUE * lightFactor;
	}
	...
	for(uint32_t iTriangle = 0; iTriangle < 12; ++iTriangle) {
		...
		if(lightFactor > 0)
			continue;
		if(triangle2dList[iTriangle].A.x < 0 && triangle2dList[iTriangle].B.x < 0 && triangle2dList[iTriangle].C.x < 0) 
			continue;
		if(triangle2dList[iTriangle].A.y < 0 && triangle2dList[iTriangle].B.y < 0 && triangle2dList[iTriangle].C.y < 0) 
			continue;
		if( triangle2dList[iTriangle].A.x >= offscreenMetricsI.x
		 && triangle2dList[iTriangle].B.x >= offscreenMetricsI.x
		 && triangle2dList[iTriangle].C.x >= offscreenMetricsI.x
		 )
			continue;
		if( triangle2dList[iTriangle].A.y >= offscreenMetricsI.y
		 && triangle2dList[iTriangle].B.y >= offscreenMetricsI.y
		 && triangle2dList[iTriangle].C.y >= offscreenMetricsI.y
		 )
			continue;
	...
	::textDrawAlignedFixedSize(offscreenView, applicationInstance.TextureFontMonochrome.View, fontAtlasView.metrics(), lineOffset = offscreenMetrics.y / 16 - 1, offscreenMetrics, sizeCharCell, textLine2, ::llc::SColorBGRA{0, framework.FrameInfo.FrameNumber % 0xFFU, 0, 0xFFU});	--lineOffset;
	return 0;																																																
}
